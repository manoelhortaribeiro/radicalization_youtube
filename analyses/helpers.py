...
colors = {"Alt-right": "#f93e49", "Alt-lite": "#019441", "Intellectual Dark Web": "#35598a", "Control": "#888888"}

bins = [(0, 1146398400000),  # - Apr 30,06
...
bins_y_o = [(0, 1146398400000),  # - Apr 30,06
            (1146398400000, 1177934400000),  # Apr 30,06 - Apr 30,07
            (1177934400000, 1209556800000),  # Apr 30,07 - Apr 30,08
            (1209556800000, 1241092800000),  # Apr 30,08 - Apr 30,09
            (1241092800000, 1272628800000),  # Apr 30,09 - Apr 30,10
...
bins_t_o = ["", "2006", "2007", "2008", "2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018"]
...
def jaccard(x, y):
    ...
    tmp["ocof"] = tmp["intersection"] / min(tmp["x"], tmp["y"])
...
def add_user_to_bin(b, user, timestamp_v, is_list=False):
...
def populate_bin_with_channel(channel, channel_dict, bin_dict):
    if channel in channel_dict:
        for user in channel_dict[channel]:
            add_user_to_bin(bin_dict, user["user_id"], user["timestamp"])


def add_user_categories_to_bin(b, user, category, timestamp_v):
    for lower, upper in b.keys():
        if upper > timestamp_v >= lower:
            user_at_b = b[(lower, upper)].get(user, {"Alt-right": 0, "Alt-lite": 0, "Intellectual Dark Web": 0,
                                                     "Control": 0})
